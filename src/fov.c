...
#define FOV_DEFINE_OCTANT(signx, signy, rx, ry, nx, ny, nf, apply_edge, apply_diag)             \
    static void fov_octant_##nx##ny##nf(                                                        \
        ...
        fov_settings_type *settings = data->settings;                                           \
                                                                                                \
        if (dx == 0) {                                                                          \
            fov_octant_##nx##ny##nf(data, dx+1, z, start_slope, end_slope);                        \
            return;                                                                             \
        ...
        for (dy = dy0; dy <= dy1; ++dy) {                                                       \
            ...
            if (settings->opaque(data->map, data->world, x, y, z, data->source_wx, data->source_wy)) {                                         \
                ...
                if (prev_blocked == 0) {                                                        \
                    ...
                    fov_octant_##nx##ny##nf(data, dx+1, z, start_slope, end_slope_next);           \
                }                                                                               \
                prev_blocked = 1;                                                               \
            } else {                                                                            \
                if (apply_edge || dy > 0) {                                                     \
...
static void _fov_circle(fov_private_data_type *data, int source_z) {
    /*
     ...
     *   /mmy|mpy\
     */
    fov_octant_ppn(data, 1, source_z, (float)0.0f, (float)1.0f);
    fov_octant_ppy(data, 1, source_z, (float)0.0f, (float)1.0f);
    fov_octant_pmn(data, 1, source_z, (float)0.0f, (float)1.0f);
    fov_octant_pmy(data, 1, source_z, (float)0.0f, (float)1.0f);
    fov_octant_mpn(data, 1, source_z, (float)0.0f, (float)1.0f);
    fov_octant_mpy(data, 1, source_z, (float)0.0f, (float)1.0f);
    fov_octant_mmn(data, 1, source_z, (float)0.0f, (float)1.0f);
    fov_octant_mmy(data, 1, source_z, (float)0.0f, (float)1.0f);
}
...
#define BEAM_DIRECTION(d, p1, p2, p3, p4, p5, p6, p7, p8, z)   \
    if (direction == d) {                                   \
        end_slope = betweenf(a, 0.0f, 1.0f);                \
        fov_octant_##p1(&data, 1, z, 0.0f, end_slope);         \
        fov_octant_##p2(&data, 1, z, 0.0f, end_slope);         \
        if (a - 1.0f > FLT_EPSILON) { /* a > 1.0f */        \
            start_slope = betweenf(2.0f - a, 0.0f, 1.0f);   \
            fov_octant_##p3(&data, 1, z, start_slope, 1.0f);   \
            fov_octant_##p4(&data, 1, z, start_slope, 1.0f);   \
        }                                                   \
        if (a - 2.0f > FLT_EPSILON) { /* a > 2.0f */        \
            end_slope = betweenf(a - 2.0f, 0.0f, 1.0f);     \
            fov_octant_##p5(&data, 1, z, 0.0f, end_slope);     \
            fov_octant_##p6(&data, 1, z, 0.0f, end_slope);     \
        }                                                   \
        if (a - 3.0f > FLT_EPSILON) { /* a > 3.0f */        \
            start_slope = betweenf(4.0f - a, 0.0f, 1.0f);   \
            fov_octant_##p7(&data, 1, z, start_slope, 1.0f);   \
            fov_octant_##p8(&data, 1, z, start_slope, 1.0f);   \
        }                                                   \
    }

#define BEAM_DIRECTION_DIAG(d, p1, p2, p3, p4, p5, p6, p7, p8, z)  \
    if (direction == d) {                                       \
        start_slope = betweenf(1.0f - a, 0.0f, 1.0f);           \
        fov_octant_##p1(&data, 1, z, start_slope, 1.0f);           \
        fov_octant_##p2(&data, 1, z, start_slope, 1.0f);           \
        if (a - 1.0f > FLT_EPSILON) { /* a > 1.0f */            \
            end_slope = betweenf(a - 1.0f, 0.0f, 1.0f);         \
            fov_octant_##p3(&data, 1, z, 0.0f, end_slope);         \
            fov_octant_##p4(&data, 1, z, 0.0f, end_slope);         \
        }                                                       \
        if (a - 2.0f > FLT_EPSILON) { /* a > 2.0f */            \
            start_slope = betweenf(3.0f - a, 0.0f, 1.0f);       \
            fov_octant_##p5(&data, 1, z, start_slope, 1.0f);       \
            fov_octant_##p6(&data, 1, z, start_slope, 1.0f);       \
        }                                                       \
        if (a - 3.0f > FLT_EPSILON) { /* a > 3.0f */            \
            end_slope = betweenf(a - 3.0f, 0.0f, 1.0f);         \
            fov_octant_##p7(&data, 1, z, 0.0f, end_slope);         \
            fov_octant_##p8(&data, 1, z, 0.0f, end_slope);         \
        }                                                       \
    }
...
void fov_beam(fov_settings_type *settings, void *map, void *source,
    ...
    BEAM_DIRECTION(FOV_EAST, ppn, pmn, ppy, mpy, pmy, mmy, mpn, mmn, source_z);
    BEAM_DIRECTION(FOV_WEST, mpn, mmn, pmy, mmy, ppy, mpy, ppn, pmn, source_z);
    BEAM_DIRECTION(FOV_NORTH, mpy, mmy, mmn, pmn, mpn, ppn, pmy, ppy, source_z);
    BEAM_DIRECTION(FOV_SOUTH, pmy, ppy, mpn, ppn, mmn, pmn, mmy, mpy, source_z);
    BEAM_DIRECTION_DIAG(FOV_NORTHEAST, pmn, mpy, mmy, ppn, mmn, ppy, mpn, pmy, source_z);
    BEAM_DIRECTION_DIAG(FOV_NORTHWEST, mmn, mmy, mpn, mpy, pmy, pmn, ppy, ppn, source_z);
    BEAM_DIRECTION_DIAG(FOV_SOUTHEAST, ppn, ppy, pmy, pmn, mpn, mpy, mmn, mmy, source_z);
    BEAM_DIRECTION_DIAG(FOV_SOUTHWEST, pmy, mpn, ppy, mmn, ppn, mmy, pmn, mpy, source_z);
}
